...
;
  ...
  function handleRemoveFromBookmarks() {
    // console.log("removed");
    toast.warning("Remove from Favorites.")
  ...
  return (
    ...
    >
      ...
      <button
        onClick={
          favoritesList.some((item) => item._id === noteItem._id)
      ...
      >
        {favoritesList.some((item) => item._id === noteItem._id) ? (
          ...
          <FaRegBookmark className="absolute top-2 right-2 text-lg sm:text-xl md:text-2xl lg:text-3xl text-blue-500 cursor-pointer" />
        )}
      </button>
      {/* BookmarkEnd */}
      <img
